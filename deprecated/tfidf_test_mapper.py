...
def cal_tfidf(tf_dict):
    ...
    for word, tf in tf_dict.items():
        if word in idf_dict:
            rs_dict[index_dict[word]] = idf_dict[word] * (1 + math.log(float(tf))) # / idf * (1 + log(tf))
...
def main(separator='\t'):   
    ...
    for doc in docs:
        ...
        feature_str = gen_format_values(cal_tfidf(tf_dict))
        if 'TEST' in class_str and len(feature_str) > 0:    # feature not empty
            print '%s%s%s' % (doc[0], separator, feature_str)
